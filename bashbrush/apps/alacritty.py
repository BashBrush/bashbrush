...
def apply_theme(palette: List[Tuple[int,int,int]], wallpaper_path: Path, paths: Paths) -> None:
    print("\n--- Processing Alacritty ---")
    paths.alacritty_themes.mkdir(parents=True, exist_ok=True)
    normal, brights = ensure_16(palette)
    name_map = ["black","red","green","yellow","blue","magenta","cyan","white"]

    theme_lines = [
        f"# Alacritty theme generated by BashBrush for {wallpaper_path.name}",
        "[colors.primary]",
    ...
    theme_path = (paths.alacritty_themes / theme_filename).resolve()
    theme_path.write_text("\n".join(theme_lines) + "\n", encoding="utf-8")
    print(f"Exported Alacritty theme to: {theme_path}")
